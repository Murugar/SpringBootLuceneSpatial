package com.iqmsoft.boot.lucene.service;
...
import com.iqmsoft.boot.lucene.pojo.SearchResult;
...
public class SpatialSearch {
	...
	public SearchResult searchCircle(double radius, Point center) throws IOException {
		...
		SpatialArgs args = new SpatialArgs
				(SpatialOperation.IsWithin, ctx.makeCircle(center.getX(), center.getY(),
		...
		TopDocs topDocs = searcher.
				search(filter, indexReader.numDocs());
	...
	public SearchResult searchBBox(Double minLat, Double minLng, Double maxLat, Double maxLng) 
			throws IOException {
		setSearchPath();
	...
	public SearchResult searchPolygon(List<Point> points) throws IOException, ParseException {
		...
		String joinedPoints = points.stream().map(x -> x.getX() + " " + x.getY()).
		...
		Query filter = strategy.makeQuery(args);
		TopDocs topDocs = searcher.search(filter, indexReader.numDocs());
